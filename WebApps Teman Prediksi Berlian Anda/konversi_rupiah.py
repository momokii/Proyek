...
class KonversiRupiah:
    ...
    def get_last_day_konversi_rate(self):
        ...
        try:
            ...
            try:
                # karena hari sudah ganti tapi nilai belum ada untuk hari kemarinnya, maka coba ambil untuk kemarin lusa
                kemarin = float(self.response['Time Series FX (Daily)'][str(LASTDAY - datetime.timedelta(days=1))]['4. close'])
                '''
                di sini pasang try-catch lagi karena ada kemungkinan terjadi pada hari selasa pada try awal gagal karena nilai hari senin belum keluar, otomatis jika masuk ke sini dan jalankan try ini maka akan cari hari minggu karena senin - 1 -> minggu, dan itu sudah pasti gagal, karena tidak ada transaksi ( record weekend ) di hari minggu
                Maka perlu ada nested try-catch disini untuk check weekend jika bagian ini gagal maka akan berusaha ambil hari jumatnya minggu kemarin dengan di bawah
                '''
            except:
                self.LASTDAY = LASTDAY - datetime.timedelta(days=1) # kurangi LASTDAY dengan -1, agar menyamai input lastday pada TRY bagian ini ( variabel kemarin pada try di atas )
                # kemudian dalam pencariannya "karena dipastikan weekend" maka cari hari dengan check weekend untuk dapat hari jumat sebelumnya
